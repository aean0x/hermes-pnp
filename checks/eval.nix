...
in
{
  modules = pkgs.runCommand "hermes-pnp-modules-eval" { } ''
    ...
    test "${containerConfig.services.hermes-webui.extraEnvironment.HERMES_PYTHON}" = "/data/toolbox/bin/python3"
    test "${containerConfig.services.hermes-webui.extraEnvironment.PIP_USER}" = "1"
    test "${toString (lib.hasInfix "HERMES_PYTHON=/data/toolbox/bin/python3" containerConfig.systemd.services.hermes-webui.preStart)}" = "1"
    test "${toString (lib.hasInfix "HERMES_PYTHON=/data/toolbox/bin/python3" (lib.concatStringsSep " " containerConfig.services.hermes-agent.container.extraOptions))}" = "1"
