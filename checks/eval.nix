...
let
  ...
  ratioConfig = eval [
    ../examples/composer.nix
    {
      services.hermesPnP.models.default.compression_ratio = 0.5;
...
in
{
  modules = pkgs.runCommand "hermes-pnp-modules-eval" { } ''
    ...
    # Single quotes: ''${VAR} escapes interpolation in an indented string, and
    # the shell must not expand either side of the comparison.
    ...
    # Same escaping rule as the host gbrain assertion above.
  ...
  options = pkgs.runCommand "hermes-pnp-options-assert" { } ''
    ...
    test "${toString (optionsEval.config.services.hermesPnP.models.default.compression_ratio == 0.26)}" = "1"
    ...
    test "${toString (lib.elem "Broad-subject conceptual or deep research" optionsEval.config.services.hermesPnP.models.default.best_for)}" = "1"
    test "${toString (lib.elem "Published outbound voice (email, social media, etc)" optionsEval.config.services.hermesPnP.models.default.best_for)}" = "1"
  ...
  examples = pkgs.runCommand "hermes-pnp-examples-eval" { } ''
    ...
    test "${toString (ratioConfig.services.hermes-agent.settings.compression.model_thresholds.${ratioConfig.services.hermesPnP.models.default.model} == 0.5)}" = "1"
