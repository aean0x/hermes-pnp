...
class TargetTier(unittest.TestCase):
    ...
    def test_floor_promotes_auxiliary(self) -> None:
        with patch.object(self.mod, "_classify", return_value="auxiliary"):
            long = (
    ...
    def test_ack_stays_auxiliary(self) -> None:
        name, reason = self.mod._target_tier("s4", "ok", [])
        self.assertEqual(name, "auxiliary")
